...
void Window::draw_minimap(SDL_Renderer *r) {
  ...
  SDL_SetRenderDrawColor(r, 0, 255, 0, 255);
  SDL_RenderDrawLine(r, x1, y1, x2, y2); // TODO Был FILL line
}

std::shared_ptr<SDL_Texture> Window::load_texture(const char *filename) {
  
  std::shared_ptr<SDL_Texture> result;
  result = std::shared_ptr<SDL_Texture>(IMG_LoadTexture(_renderer.get(), filename), SDL_DestroyTexture);
  if (result == nullptr)
    throw std::runtime_error(
...
Window::Window(int w, int h) : _width(w), _height(h) {
  ...
  SDL_QueryTexture(_wall_tex.get(), nullptr, nullptr,
                   &_wall_tex_width, &_wall_tex_height);
  SDL_QueryTexture(_door_tex.get(), nullptr, nullptr,
...
void Window::render(SDL_Renderer *r) {
  ...
  SDL_SetRenderDrawColor(r, 62, 62, 250, 255);
  ...
  for (int col = 0; col < _width; ++col) {
    double gamma = atan2(col - _width/2.0, ds);
...
//    SDL_RenderDrawLine(r, col, _height/2 - h/2, col, _height/2 + h/2); //TODO Был комментарий!!!
    if (c == '@') {
...
void Window::cast_ray(double &rx, double &ry, double dx, double dy) {
  ...
  do {
    ...
    while (mx < 0)
      mx += _map->width();
    while (my >= _map->height())
      my -= _map->height();
    while (my < 0)
      my += _map->height();
    --limit;
  }
  while (not _map->is_wall(mx, my) and limit);
...
void Window::draw_col(SDL_Renderer *r, SDL_Texture *t,
                      int
                      col,
                      int h,
                      int tw,
  ...
  SDL_Rect src{int(floor(tx*tw)), 0, 1, th};
